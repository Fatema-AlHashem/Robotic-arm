...
def bestMove(board):
  ...
  for i in range(3):
      for j in range(3):
          if (board[i][j]==''):
              board[i][j]=ai #playerMove()
              # float(-inf) is alpha , float(inf) is beta
              score=minimax(board,0 ,float(-math.inf),float(math.inf),False)
              #undoing the board
              board[i][j]=''
              if(score >BestScore):
                 BestScore= score
  ...
  board[move.i][move.j]=ai #playerMove()
  currentPlayer=human #playerMove()
